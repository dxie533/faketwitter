				...
				username = decoded.username;
					request.post({
						...
						if(results.status === "error"){
							...
							var returnString = "<html><head><script src = '/userPage.js'></script></head><body><a href = 'http://helloworld123.cse356.compas.cs.stonybrook.edu/'>Home</a> <a href = 'http://helloworld123.cse356.compas.cs.stonybrook.edu/searchpage'>Search for posts</a> <button onclick = 'logout()'>Log out</button><br/>Add a new post:<input type = 'text' id = 'content' width = '200px' height = '200px'></input><br/>Add media:<br/><select id = 'mediaSelection' multiple></select><button onclick = 'addItem()'>Add post</button><br/>Delete specific item with id:<input type = 'text' id = 'deleteContent'></input><button onclick = 'deleteSelected()'>Delete Item</button><br/><div>Follow a specific user:<input type='text' id = 'followField'></input><button onclick = 'follow()'>Follow</button></div><br/><div>Unfollow a specific user:<input type='text' id = 'unfollowField'></input><button onclick = 'unfollow()'>Unfollow</button></div> <div id = 'addResult'></div><br/><div id = 'deleteResult'></div><h1>Your 100 latest posts</h1><div id = 'useritems'>"+generatedPostString+"</div></body><script>updateAvailableMedia()</script></html>";
